...
export function DeviceCard({ device, index }: { device: Device, index?: number }) {
  ...
  return (
    <Card className="group relative border border-border/80 bg-card/30 backdrop-blur-md shadow-lg hover:shadow-primary/5 overflow-hidden transform-gpu translate-z-0">
      ...
      {index && (
        <div className="absolute top-3 right-4 z-20 select-none pointer-events-none opacity-90">
      ...
      <div className="m-5 mb-0 rounded-xl border border-border/30 bg-muted/10 p-5 flex items-start justify-between gap-3">
        ...
        <Button
          variant="ghost"
          size="icon"
          className="h-7 w-7 shrink-0 rounded-full text-muted-foreground/30 hover:text-destructive hover:bg-destructive/10 mt-0.5"
      ...
      <div className="px-4 pb-4 flex gap-2">
        <Button
          ...
          variant="outline"
          className="flex-1 h-10 text-[12px] font-black uppercase tracking-wider border-border/40 bg-transparent text-foreground/70 hover:bg-primary hover:text-white hover:border-primary transform-gpu"
        >
          Manage
        </Button>
        <Button
